...
@pytest.fixture
def high_tpot() -> MetricSeriesSnapshot:
    return MetricSeriesSnapshot(tpot_p95_seconds=0.5)


@pytest.fixture
def high_tpot_low_gen() -> MetricSeriesSnapshot:
    return MetricSeriesSnapshot(tpot_p95_seconds=0.5, generation_tokens_per_second=20.0)


@pytest.fixture
def high_tpot_low_gen_normal_ttft() -> MetricSeriesSnapshot:
    return MetricSeriesSnapshot(tpot_p95_seconds=0.5, generation_tokens_per_second=20.0, ttft_p95_seconds=0.3)
...
class TestTPOTBottleneckRule:
    ...
    def test_no_finding_below_threshold(self, rule: TPOTBottleneckRule) -> None:
        assert rule.run(MetricSeriesSnapshot(tpot_p95_seconds=0.1)) is None

    def test_no_finding_at_boundary(self, rule: TPOTBottleneckRule) -> None:
        assert rule.run(MetricSeriesSnapshot(tpot_p95_seconds=0.19)) is None

    def test_finding_at_threshold(self, rule: TPOTBottleneckRule) -> None:
        result = rule.run(MetricSeriesSnapshot(tpot_p95_seconds=0.2))
        assert result is not None
        assert rule.severity == Severity.warning

    def test_low_confidence_tpot_only(self, rule: TPOTBottleneckRule, high_tpot: MetricSeriesSnapshot) -> None:
        assert rule.run(high_tpot).confidence == Confidence.low

    def test_medium_confidence_with_low_gen(
        self, rule: TPOTBottleneckRule, high_tpot_low_gen: MetricSeriesSnapshot
    ) -> None:
        assert rule.run(high_tpot_low_gen).confidence == Confidence.medium

    def test_high_confidence_with_all_signals(
        self, rule: TPOTBottleneckRule, high_tpot_low_gen_normal_ttft: MetricSeriesSnapshot
    ) -> None:
    ...
    def test_high_gen_does_not_boost_confidence(self, rule: TPOTBottleneckRule) -> None:
        metrics = MetricSeriesSnapshot(tpot_p95_seconds=0.5, generation_tokens_per_second=200.0)
    ...
    def test_high_ttft_does_not_contribute_ttft_normal_signal(self, rule: TPOTBottleneckRule) -> None:
        metrics = MetricSeriesSnapshot(tpot_p95_seconds=0.5, ttft_p95_seconds=5.0)
        result = rule.run(metrics)
        assert result is not None
        assert not any("TTFT p95 is normal" in s for s in result.signals)

    def test_evidence_contains_tpot(self, rule: TPOTBottleneckRule, high_tpot: MetricSeriesSnapshot) -> None:
    ...
    def test_no_finding_when_tpot_is_nan(self, rule: TPOTBottleneckRule) -> None:
        assert rule.run(MetricSeriesSnapshot(tpot_p95_seconds=float("nan"))) is None

    def test_nan_ttft_does_not_count_as_normal(self, rule: TPOTBottleneckRule) -> None:
        metrics = MetricSeriesSnapshot(
            tpot_p95_seconds=0.5, generation_tokens_per_second=20.0, ttft_p95_seconds=float("nan")
        )
        assert rule.run(metrics).confidence == Confidence.medium

    def test_nan_ttft_not_in_evidence(self, rule: TPOTBottleneckRule) -> None:
        metrics = MetricSeriesSnapshot(tpot_p95_seconds=0.5, ttft_p95_seconds=float("nan"))
        result = rule.run(metrics)
    ...
    async def test_tpot_bottleneck_with_scrape_fixture(self, rule: TPOTBottleneckRule) -> None:
        metrics = await snapshot_from_scrape_fixture("tpot-bottleneck.txt")
    ...
    async def test_tpot_bottleneck_with_prometheus_fixture(self, rule: TPOTBottleneckRule) -> None:
        metrics = await snapshot_from_prometheus_fixture("tpot-bottleneck.json")
        assert rule.run(metrics) is not None

    def test_custom_threshold(self) -> None:
        rule = TPOTBottleneckRule(high_tpot_p95=0.5)
        assert rule.run(MetricSeriesSnapshot(tpot_p95_seconds=0.49)) is None
        assert rule.run(MetricSeriesSnapshot(tpot_p95_seconds=0.5)) is not None
