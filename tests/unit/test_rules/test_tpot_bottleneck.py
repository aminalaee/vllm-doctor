...
class TestTPOTBottleneckRule:
    ...
    def test_evidence_contains_tpot(self, rule: TPOTBottleneckRule, high_tpot: Metrics) -> None:
        findings = rule.run(_CTX, high_tpot)
    ...
    def test_custom_threshold(self) -> None:
        ...
        assert rule.run(_CTX, Metrics(tpot_p95_seconds=0.49)) == []
        assert len(rule.run(_CTX, Metrics(tpot_p95_seconds=0.5))) == 1
