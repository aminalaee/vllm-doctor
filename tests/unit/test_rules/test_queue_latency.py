...
class TestQueueLatencyRule:
    ...
    def test_no_finding_below_threshold(self, rule: QueueLatencyRule) -> None:
        assert rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=0.5)) == []

    def test_no_finding_at_boundary(self, rule: QueueLatencyRule) -> None:
        assert rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=0.99)) == []

    def test_finding_at_threshold(self, rule: QueueLatencyRule) -> None:
        findings = rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=1.0))
        assert len(findings) == 1
        assert findings[0].severity == Severity.warning

    def test_low_confidence_without_waiting(self, rule: QueueLatencyRule) -> None:
        assert rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=2.0))[0].confidence == Confidence.low
    ...
    def test_high_confidence_with_waiting(self, rule: QueueLatencyRule) -> None:
        ...
        assert rule.evaluate(_CTX, current)[0].confidence == Confidence.high
    ...
    def test_no_finding_when_nan(self, rule: QueueLatencyRule) -> None:
        assert rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=float("nan"))) == []

    def test_evidence_contains_queue_time(self, rule: QueueLatencyRule) -> None:
        findings = rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=1.5))
    ...
    def test_summary_contains_queue_time(self, rule: QueueLatencyRule) -> None:
        assert "3.00" in rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=3.0))[0].summary
    ...
    def test_custom_threshold(self) -> None:
        ...
        assert rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=4.9)) == []
        assert len(rule.evaluate(_CTX, Metrics(queue_time_p95_seconds=5.0))) == 1
