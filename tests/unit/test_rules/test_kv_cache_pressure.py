...
class TestKVCachePressureRule:
    ...
    def test_evidence_contains_cache_usage(self, rule: KVCachePressureRule, high_cache: Metrics) -> None:
        findings = rule.evaluate(_CTX, high_cache)
    ...
    def test_evidence_contains_waiting_when_present(
        ...
        findings = rule.evaluate(_CTX, high_cache_with_waiting)
    ...
    def test_custom_threshold(self) -> None:
        ...
        assert rule.evaluate(_CTX, Metrics(kv_cache_usage_perc=0.74)) == []
        assert len(rule.evaluate(_CTX, Metrics(kv_cache_usage_perc=0.75))) == 1
