...
@pytest.fixture
def high_ttft() -> Metrics:
    return Metrics(ttft_p95_seconds=3.0)


@pytest.fixture
def high_ttft_stable_tpot() -> Metrics:
    return Metrics(ttft_p95_seconds=3.0, tpot_p95_seconds=0.05)


@pytest.fixture
def high_ttft_stable_tpot_waiting() -> Metrics:
    return Metrics(ttft_p95_seconds=3.0, tpot_p95_seconds=0.05, num_requests_waiting=10)
...
class TestTTFTBottleneckRule:
    ...
    def test_evidence_contains_tpot_when_present(
        ...
        findings = rule.evaluate(_CTX, high_ttft_stable_tpot)
        assert any("TPOT" in e for e in findings[0].evidence)

    def test_evidence_contains_waiting_when_present(
        self, rule: TTFTBottleneckRule, high_ttft_stable_tpot_waiting: Metrics
    ) -> None:
        findings = rule.evaluate(_CTX, high_ttft_stable_tpot_waiting)
    ...
    def test_no_finding_when_ttft_is_nan(self, rule: TTFTBottleneckRule) -> None:
        assert rule.evaluate(_CTX, Metrics(ttft_p95_seconds=float("nan"))) == []
    ...
    def test_custom_threshold(self) -> None:
        ...
        assert rule.evaluate(_CTX, Metrics(ttft_p95_seconds=4.9)) == []
        assert len(rule.evaluate(_CTX, Metrics(ttft_p95_seconds=5.0))) == 1
