...
class TestPreemptionPressureRule:
    ...
    def test_no_finding_when_zero_preemptions(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now", metrics=Metrics(num_preemptions_total=0)
        )
        assert rule.evaluate(snapshot) == []

    def test_finding_when_preemptions_nonzero(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now", metrics=Metrics(num_preemptions_total=5)
        )
        findings = rule.evaluate(snapshot)
        assert len(findings) == 1
        assert findings[0].severity == Severity.warning

    def test_medium_confidence_without_cache_signal(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now", metrics=Metrics(num_preemptions_total=10)
        )
        assert rule.evaluate(snapshot)[0].confidence == Confidence.medium

    def test_high_confidence_with_high_cache(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now",
    ...
    def test_medium_confidence_with_low_cache(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now",
    ...
    def test_evidence_contains_preemption_count(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now", metrics=Metrics(num_preemptions_total=42)
        )
        assert any("42" in e for e in rule.evaluate(snapshot)[0].evidence)

    def test_evidence_contains_cache_usage_when_high(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
            window="now",
    ...
    def test_summary_contains_preemption_count(
        self, rule: PreemptionPressureRule
    ) -> None:
        snapshot = MetricSnapshot(
