...
from vllm_doctor.rules.utils.trend import falling, rising
...
class TestTTFTTrend:
    def test_rising_signal_added(self) -> None:
        ...
        assert any("rising" in s for s in TTFTBottleneckRule().run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        result = TTFTBottleneckRule().run(Metrics(ttft_p95_seconds=3.0))
        assert not any("rising" in s for s in result.signals)

    def test_no_trend_signal_when_stable(self) -> None:
        curr, prev = Metrics(ttft_p95_seconds=3.0), Metrics(ttft_p95_seconds=2.9)
        assert not any("rising" in s for s in TTFTBottleneckRule().run(curr, previous=prev).signals)

    def test_trend_signal_contains_values(self) -> None:
        curr, prev = Metrics(ttft_p95_seconds=3.0), Metrics(ttft_p95_seconds=2.0)
        signal = next(s for s in TTFTBottleneckRule().run(curr, previous=prev).signals if "rising" in s)
...
class TestTPOTTrend:
    def test_rising_signal_added(self) -> None:
        ...
        assert any("rising" in s for s in TPOTBottleneckRule().run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        assert not any("rising" in s for s in TPOTBottleneckRule().run(Metrics(tpot_p95_seconds=0.35)).signals)

    def test_trend_signal_contains_values(self) -> None:
        curr, prev = Metrics(tpot_p95_seconds=0.350), Metrics(tpot_p95_seconds=0.200)
        signal = next(s for s in TPOTBottleneckRule().run(curr, previous=prev).signals if "rising" in s)
...
class TestKVCacheTrend:
    def test_rising_signal_added(self) -> None:
        ...
        assert any("rising" in s for s in KVCachePressureRule().run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        result = KVCachePressureRule().run(Metrics(kv_cache_usage_perc=0.95))
        assert not any("rising" in s for s in result.signals)

    def test_trend_signal_contains_percentages(self) -> None:
        curr, prev = Metrics(kv_cache_usage_perc=0.95), Metrics(kv_cache_usage_perc=0.80)
        signal = next(s for s in KVCachePressureRule().run(curr, previous=prev).signals if "rising" in s)
...
class TestQueuePressureTrend:
    def test_growing_signal_added(self) -> None:
        ...
        assert any("growing" in s for s in QueuePressureRule(high_waiting=3).run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        result = QueuePressureRule(high_waiting=3).run(Metrics(num_requests_waiting=15))
        assert not any("growing" in s for s in result.signals)

    def test_trend_signal_contains_values(self) -> None:
        curr, prev = Metrics(num_requests_waiting=15), Metrics(num_requests_waiting=5)
        signal = next(s for s in QueuePressureRule(high_waiting=3).run(curr, previous=prev).signals if "growing" in s)
...
class TestQueueLatencyTrend:
    def test_worsening_signal_added(self) -> None:
        ...
        assert any("worsening" in s for s in QueueLatencyRule().run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        result = QueueLatencyRule().run(Metrics(queue_time_p95_seconds=1.5))
        assert not any("worsening" in s for s in result.signals)

    def test_trend_signal_contains_values(self) -> None:
        curr, prev = Metrics(queue_time_p95_seconds=1.50), Metrics(queue_time_p95_seconds=1.00)
        signal = next(s for s in QueueLatencyRule().run(curr, previous=prev).signals if "worsening" in s)
...
class TestLowThroughputTrend:
    def test_declining_gen_signal_added(self) -> None:
        ...
        assert any("declining" in s for s in LowThroughputRule().run(curr, previous=prev).signals)
    ...
    def test_no_trend_signal_without_previous(self) -> None:
        result = LowThroughputRule().run(Metrics(generation_tokens_per_second=30.0))
        assert not any("declining" in s for s in result.signals)

    def test_trend_signal_contains_values(self) -> None:
        curr, prev = Metrics(generation_tokens_per_second=30.0), Metrics(generation_tokens_per_second=80.0)
        signal = next(s for s in LowThroughputRule().run(curr, previous=prev).signals if "declining" in s)
...
class TestPrefixCacheTrend:
    def test_declining_signal_added(self) -> None:
        ...
        assert any("declining" in s for s in PrefixCacheEfficiencyRule().run(curr, previous=prev).signals)

    def test_no_trend_signal_without_previous(self) -> None:
        assert PrefixCacheEfficiencyRule().run(Metrics(prefix_cache_hit_rate=0.25)).signals == []

    def test_trend_signal_contains_percentages(self) -> None:
        curr, prev = Metrics(prefix_cache_hit_rate=0.25), Metrics(prefix_cache_hit_rate=0.45)
        signal = next(s for s in PrefixCacheEfficiencyRule().run(curr, previous=prev).signals if "declining" in s)
