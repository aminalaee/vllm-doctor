...
class TestPrefixCacheEfficiencyRule:
    ...
    def test_no_finding_above_threshold(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.5))
        assert rule.evaluate(snapshot) == []

    def test_no_finding_at_threshold(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.6))
        assert rule.evaluate(snapshot) == []

    def test_finding_below_threshold(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.3))
        findings = rule.evaluate(snapshot)
        assert len(findings) == 1
        assert findings[0].severity == Severity.warning

    def test_zero_hit_rate(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.0))
        findings = rule.evaluate(snapshot)
        assert len(findings) == 1
    ...
    def test_high_confidence_very_low_rate(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.1))
    ...
    async def test_prefix_cache_efficiency_with_scrape_fixture(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = await snapshot_from_scrape_fixture("prefix-cache.txt")
    ...
    async def test_prefix_cache_efficiency_with_prometheus_fixture(self, rule: PrefixCacheEfficiencyRule) -> None:
        snapshot = await snapshot_from_prometheus_fixture("prefix-cache.json")
        assert len(rule.evaluate(snapshot)) == 1

    def test_custom_threshold(self) -> None:
        rule = PrefixCacheEfficiencyRule(min_hit_rate=0.8)
        assert rule.evaluate(MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.8))) == []
        assert len(rule.evaluate(MetricSnapshot(window="now", metrics=Metrics(prefix_cache_hit_rate=0.79)))) == 1
