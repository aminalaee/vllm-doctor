...
impl Rule for TtftBottleneckRule {
    ...
    fn run(&self, metrics: &MetricSeriesSnapshot) -> Option<FindingData> {
        ...
        if !ttft.is_finite() || ttft < self.cfg.high_ttft_p95 {
            return None;
        }
...
mod tests {
    ...
    fn no_finding_when_ttft_low() {
        assert!(rule().run(&snapshot(1.0, 0.1, 5.0)).is_none());
    }

    #[test]
    fn low_confidence_when_ttft_high_only() {
        let finding = rule().run(&snapshot(3.0, 0.5, 0.0)).unwrap();
        assert_eq!(finding.confidence, Confidence::Low);
    }

    #[test]
    fn high_confidence_when_ttft_high_with_stable_tpot_and_waiting() {
        let finding = rule().run(&snapshot(3.0, 0.1, 5.0)).unwrap();
