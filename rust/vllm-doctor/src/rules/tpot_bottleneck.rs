...
impl Rule for TpotBottleneckRule {
    ...
    fn run(&self, metrics: &MetricSeriesSnapshot) -> Option<FindingData> {
        let tpot = metrics.tpot_p95_seconds.value()?;
        if !tpot.is_finite() || tpot < self.cfg.high_tpot_p95 {
            return None;
        }

        let ttft = metrics.ttft_p95_seconds.value();
        ...
        if ttft_normal {
            signals.push("TTFT p95 is normal — bottleneck is in decode, not prefill".to_string());
        }
...
mod tests {
    ...
    fn no_finding_when_tpot_low() {
        assert!(rule().run(&snapshot(0.1, 1.0, 100.0)).is_none());
    }

    #[test]
    fn low_confidence_when_tpot_high_only() {
        let finding = rule().run(&snapshot(0.3, 5.0, 100.0)).unwrap();
        assert_eq!(finding.confidence, Confidence::Low);
    }

    #[test]
    fn high_confidence_when_tpot_high_with_low_gen_and_normal_ttft() {
        let finding = rule().run(&snapshot(0.3, 1.0, 10.0)).unwrap();
