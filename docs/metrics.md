...
| `vllm:gpu_cache_usage_perc`          | Fraction of GPU KV cache currently in use (0.0–1.0); `n/a` on idle servers until at least one request has been processed | Both       |
...
| `vllm:time_per_output_token_seconds` | Histogram of time per output token during decode                                                                         | Prometheus |
...
- Latency histograms (`time_to_first_token_seconds`, `time_per_output_token_seconds`) require Prometheus mode — direct scrape mode returns `None` for these fields.
