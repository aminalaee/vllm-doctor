...
- When several series match a metric, reported counts and throughput are
  summed; utilization and percentile-style values use the highest observed
  value. Use the model filter to scope collection to one `model_name`.
- Latency histograms (`time_to_first_token_seconds`,
  `request_time_per_output_token_seconds`) require Prometheus mode and are not
  reported in direct scrape mode.
