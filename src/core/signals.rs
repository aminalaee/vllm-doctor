...
use crate::core::metrics::{MODEL_LABEL, MetricSeriesSnapshot, evaluate_direct};

// `Signal` is generated by `define_metrics!` in `crate::core::metrics` and re-exported
// here so that the public path `crate::core::signals::Signal` stays stable.
pub use crate::core::metrics::Signal;
...
impl<'a> SignalGraph<'a> {
    ...
    pub fn evaluate(&self, signal: Signal) -> Option<f64> {
        // Direct and ratio signals are generated from the metric table.
        if let Some(value) = evaluate_direct(signal, self.snapshot) {
            return Some(value);
        }
        // Computed signals are derived manually below.
        match signal {
            ...
            // Direct/ratio signals already handled by evaluate_direct above.
            _ => None,
        }
    }
    ...
    pub fn replica_label(&self) -> Option<&str> {
        crate::core::metrics::detect_replica_label(self.snapshot)
    }
