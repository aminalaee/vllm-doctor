...
use crate::cli::reports::format::format_value;
use crate::cli::reports::{RenderOptions, Report};
use crate::core::metrics::{all_specs, detect_replica_label};
use crate::core::models::{BottleneckKind, Finding, Health, Severity};
...
fn render_notices(report: &Report, opts: &RenderOptions, out: &mut String) {
    let notices = crate::cli::reports::notices::resolve_notices(&report.diagnosis);
    if notices.is_empty() {
        return;
    }
...
fn render_metrics(report: &Report, opts: &RenderOptions, out: &mut String) {
    ...
    for spec in all_specs() {
        let spec: &dyn crate::core::metrics::MetricSpec = spec.as_ref();
...
fn render_replica_metrics(report: &Report, opts: &RenderOptions, label: &str, out: &mut String) {
    ...
    let mut specs_with_data: Vec<&dyn crate::core::metrics::MetricSpec> = Vec::new();
    ...
    let fmt_cell = |spec: &dyn crate::core::metrics::MetricSpec, replica: &str| -> String {
...
mod tests {
    ...
    fn scrape_mode_renders_notice_in_verbose() {
        ...
        result.context = result
            .context
            .with_metrics_source(crate::core::models::MetricsSource::DirectScrape);
        let report = Report::new(result);
